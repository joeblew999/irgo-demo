...
| `go.mod` replace | `myapp/go.mod` | `=> github.com/joeblew999/irgo v0.4.0-androidapi21.12` |
| CLI install | `mise/tasks/dev.toml` → `env:tools` | local checkout (`$IRGO_FORK_DIR`) or clone of pinned `$IRGO_FORK_TAG`, then `irgo install-tools`; released `v0.3.1` fallback |
...
| 5 | irgo CLI from fork — local checkout (`IRGO_FORK_DIR`) or clone of pinned `IRGO_FORK_TAG` (`v0.4.0-androidapi21.12`) so CI/fresh machines get the fixed CLI | `env:tools` | released CLI lacks the fixes | released irgo ≥ fixes → plain `go install github.com/stukennedy/irgo/cmd/irgo@<tag>`; fork vars become optional |
